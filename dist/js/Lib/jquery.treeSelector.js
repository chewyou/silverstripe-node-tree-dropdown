(function ($) {
  jQuery.fn.treeSelector = function (tree, defaultValues, onChange, params) {
    ...
    var buildTree = function (node, level, randId) {
      ...
      if (hasChildren && options.showToggles === true) {
        var liTitleToggle = $(document.createElement('div'));
        liTitleToggle.attr({
          class: 'font-icon-down-dir toggle-node level-'+level,
          'data-value': node.value
        })
        liBox.append(liTitleToggle)
      }
      ...
      if ($selector && values && Array.isArray(values)) {
        ...
        for (var k = 0; k < values.length; k++) {
          ...
          if (item) {
            ...
            faClose.addClass('font-icon-cancel')
    ...
    var bindEvents = function ($selector) {
       ...
       * click [x] icon
       */
      $selector.on('click', 'span.font-icon-cancel', function (e) {
        if (options.disabled) {
          return false
        }
    ...
    // TODO: Fix issue when opening the wrapper more than once stopping the show and hide from working
    var allowToggle = function () {
      $('.toggle-node').on('click', function () {
        var children = $(this).siblings('ul');
        var isOpen = $(this).hasClass('font-icon-down-dir');
